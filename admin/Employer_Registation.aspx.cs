...
public partial class Employer_Registation : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            {
                if (Request.QueryString["cat"] != "exter")
                {
                    inext.Visible = false;
                }
                else
                ...
                ldate.Text = DateTime.Now.ToShortDateString().ToString();
        ...
        if (TextBox1.Text != "" && TextBox3.Text != "" && TextBox7.Text != "" && TextBox2.Text != "" && TextBox9.Text != "" && uname.Text!="" && password.Text!="")
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    int id11 = Int32.Parse(dr[0].ToString()) + 1;
                    dr.Close();
                    m.CommandText = "select  count(*)  from  clogin_detail where Client_number like 'SPC%'";
                    dr80 = m.ExecuteReader();
                    dr80.Read();
                    string clnum5 = "SPC" + Convert.ToString(Int32.Parse(dr80[0].ToString()) + 1); 
                    dr80.Close();
                    m.Parameters.AddWithValue("@id_txt", id11);
                    
                    m.Parameters.AddWithValue("@Client_number", clnum5);
                    m.Parameters.AddWithValue("@branch_txt", branch.Text.ToString().Trim().Replace("'", "''"));
                    ...
                    m.Parameters.AddWithValue("@t_n_o2", other.Text.ToString().Trim().Replace("'", "''"));
                    ...
                    m.Parameters.AddWithValue("@terms_txt", terms.Text.ToString().Trim().Replace("'", "''"));
                    m.Parameters.AddWithValue("@start_date", DateTime.Now.ToString());
                    m.Parameters.AddWithValue("@end_date",  Convert.ToDateTime(ldate.Text).ToShortDateString());
                    ...
                    m.Parameters.AddWithValue("@dev_by", developby.Text);
                    m.ExecuteNonQuery();
