...
public partial class Resume_Search_Form : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            if (Session["resume_search"] == "yes")
            {
                //ListBox2.SelectedValue = "Select";
            }
            else
            {
                ScriptManager.RegisterClientScriptBlock(Page, typeof(Page), "script", "alert('You are not authorised for Resume Search. please contact us')", true);
